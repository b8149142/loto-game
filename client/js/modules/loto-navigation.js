...
export async function openLotoRoom(ws, roomId) {
  ...
  if (isGameStarted.data == true) {
    ...
    if (isPlayerInRoom.data == true) {
      ...
      impLotoGame.openGamePage(null, null, null, roomId);
    } else {
    ...
    body.innerHTML = `<div class="loto-room-page">
      <div class="loto-room-content">
        ...
        <div class="loto-room__gameinfo loto-gameinfo">
          <p class="loto-gameinfo__bet">Ставка: <span>0M</span></p>
          <p class="loto-gameinfo__online">Онлайн: <span>0</span></p>
          <p class="loto-gameinfo__bank">Банк: <span>0M</span></p>
        </div>
        <div class="room-jackpot">
         Джекпот: <span class="room-jackpot-sum">0</span>
        </div>
...
function updateAllRoomsBet(betArr) {
  ...
  if (mainPage) {
    for (let roomId = 1; roomId <= 5; roomId++) {
      ...
      if (lotoRoomBet) {
        lotoRoomBet.innerHTML = betArr[`room${roomId}`].toFixed(2);
      }
    }
  }
}
...
function updateAllRoomsJackpot(jackpots) {
  ...
  if (mainPage) {
    for (let roomId = 1; roomId <= 5; roomId++) {
      ...
      if (lotoRoomJackpot) {
        lotoRoomJackpot.innerHTML = jackpots[`room${roomId}`].toFixed(2);
      }
    }
  }
}
...
function updateBank(bank) {
  ...
  if (lotoGameInfo) {
    ...
    lotoBank.querySelector("span").innerHTML = `${bank.toFixed(2)}М`;
  }
}
...
function updateJackpot(jackpot) {
  ...
  if (lotoJackpot) {
    lotoJackpot.innerHTML = `${jackpot.toFixed(2)}М`;
  }
}
